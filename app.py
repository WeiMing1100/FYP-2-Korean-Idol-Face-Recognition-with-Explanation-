...
# @st.cache_resource # don't cache cuz every GradCAM will have different image input
def get_grad_cam():
...
if uploaded_image is None:
    ...
    with st.spinner("Loading... Please wait."):
        ...
        if img_bgr is None or img_bgr.size == 0:
            st.error("Image not loaded properly! Please try uploading another image.")
        else:
            ...
            if extracted_face is None:
                st.error("The model failed to detect any faces in the image. Please try uploading another image")
                ...
                if is_kpop_idol:
                    ...
                    if top_score is not None and top_score < 0.2: # weak focus
                        ...
                        st.error("The model failed to detect facial importance. Please try uploading another image.")
